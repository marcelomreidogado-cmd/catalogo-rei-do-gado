...
await page.locator('[data-order]').first().click();await page.locator('[name=finalTotal]').fill('55,90');await page.locator('#modal [data-order-status=done]').click();await page.waitForSelector('#final-total-form',{state:'hidden'});await page.locator('[data-admin-tab=customers]').click();assert.match(await page.locator('#admin-content').innerText(),/QA REVISAO coronel/);
